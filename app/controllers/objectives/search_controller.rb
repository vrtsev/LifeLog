class Objectives::SearchController < ObjectivesController
  def index
    @goals   = find_goals
    @tags    = find_tags
    @actions = find_actions.page(params[:actions_page])
    @tasks   = find_tasks.page(params[:tasks_page])
  end
  ...
  def find_goals
    ...
    GoalsSearchQuery.new(scope, params[:query], params[:goal]).results
  end
  ...
  def find_actions
    ...
    ActionsSearchQuery.new(scope, params[:query], params[:action]).results
  end
  ...
  def find_tasks
    ...
    TasksSearchQuery.new(scope, params[:query], params[:task]).results
  end
end
